...
- 관리자 전용 `/내전 세팅`
  ...
  - 같은 카테고리 안에 `내전-모집` 채널과 코어 임베드 생성
...
- 모집 채널
  - 즉시 모집 이름: `🏠ㆍ협곡 내전 1`, `🏠ㆍ아람 내전 2`처럼 같은 카테고리의 현재 열린 협곡·아람 방이 번호를 함께 사용
  - 누적 생성 횟수가 아니라 현재 열린 방만 기준으로 하며, 방이 닫히면 비어 있는 가장 작은 번호를 다시 사용
  - 예약 모집 이름: `⏰ㆍ12월 31일 내전`
...
- 올 소환
  ...
  - 대기열에 한 명 이상 있으면 사용 가능
  - 10명 이하면 대기열 전원, 11명 이상이면 선착순 10명 중 현재 서버 음성 채널에 접속한 멤버만 이동
...
- `src/services/queuePresentation.ts` — 참가 순번과 선착순 대상 계산
...
자동 테스트는 DB 생성 제한과 현재 열린 채널 번호 재사용, 무제한 대기열 순서, 중복 참가, 서버 쿨타임, 1회 소환 잠금, 예약 시간/타임존, 단계별 임베드 명단, 임베드 항목 추가·삭제·빈 설정·이전 버전 호환, 모달 셀렉트와 채널 권한, 버튼 ID 라우팅을 검사합니다.
