...
- 모집 임베드
  ...
  - 참가/이탈 때 즉시 메시지 갱신
  - 첫 행: `신청하기`, `쫄튀하기`, `전체 멘션`, `전체 소환`
  - 둘째 행: `마감하기`(마감 후 `재오픈`), `팀 짜기`, `수동 추가`, `수동 제외`, `삭제`
- 신청과 라이엇 ID
...
- 수동 대기열 관리
  - Discord 관리자와 `내전관리자` 역할만 수동 추가·제외 가능하며 일반 모집 생성자는 사용할 수 없음
  - 수동 추가 모달의 Discord 서버 멤버 선택창에서 닉네임 일부를 검색한 뒤 정확한 사용자를 선택
  - 수동 추가·제외는 참가 신청 마감 상태에서도 가능하며 중복 참가와 40명 상한은 그대로 검사
  - 수동 제외도 모달로 열리며 현재 대기열 순번과 서버 표시 이름을 선택 메뉴에 표시
  - Discord 선택 메뉴의 25개 제한에 맞춰 26명 이상이면 모달 안에서 `1~25`, `26~40` 두 메뉴로 나눠 전원을 표시
- 전체 멘션
...
- 삭제
  ...
  - 첫 클릭 후 비공개 `정말 삭제`·`취소` 버튼으로 한 번 더 확인
