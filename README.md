...
자동 테스트는 DB 생성 제한과 게임별 열린 채널 순번 재사용, 즉시 모집 시작 시간, 무제한 대기열 순서, 중복 참가, 서버 쿨타임, 1회 소환 잠금, 예약 시간/타임존, 단계별 임베드 명단, 임베드 항목 추가·삭제·빈 설정·이전 버전 호환, 모달 셀렉트와 채널 권한, 버튼 ID 라우팅을 검사합니다.
